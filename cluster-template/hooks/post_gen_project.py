...
def cleanup_infrastructure_directories(cloud_provider):
    ...
    for item in project_dir.iterdir():
        if item.is_dir() and item.name.startswith("infrastructure-") and item != cloud_provider_dir:
...
def extend_gitignore():
    """Extend the .gitignore file with the .install directory."""
    
    project_dir = Path.cwd()
    gitignore_file = project_dir / ".gitignore"
    
    with open(gitignore_file, "a") as f:
        f.write("\n# PHD stack installation helpers\n.install/\n")
...
def main():
    ...
    if not infrastructure_dir.exists():
        print(f"Infrastructure directory {infrastructure_dir} not found, skipping formatting")
        return
