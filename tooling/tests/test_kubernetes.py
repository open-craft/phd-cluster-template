...
class TestKubernetesClient:
    ...
    @mock.patch("phd.kubernetes.config.load_kube_config")
    @mock.patch("phd.kubernetes.get_logger", return_value=mock.Mock())
    @mock.patch("phd.kubernetes.utils.create_from_dict")
    ...
    def test_apply_manifest_success(
        ...
        mock_create.assert_called_once_with(
            mock_api_client_instance, doc, namespace="test-ns"
        )
    ...
    def test_apply_manifest_multiple_documents(
        ...
        doc1 = {"apiVersion": "v1", "kind": "ConfigMap", "metadata": {}}
        doc2 = {"apiVersion": "v1", "kind": "Secret", "metadata": {}}
    ...
    def test_apply_manifest_failure(
        ...
        doc = {"apiVersion": "v1", "kind": "ConfigMap", "metadata": {}}
        mock_yaml_load.return_value = [doc]
        mock_create.side_effect = Exception("Apply failed")

        k8s_client = KubernetesClient()

        with pytest.raises(KubernetesError, match="Failed to apply manifest"):
            k8s_client.apply_manifest(manifest, namespace="test-ns")
