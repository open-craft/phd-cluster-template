...
from phd.utils import check_command_installed, get_logger, log_success
...
def _delete_argocd_application(instance_name: str) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "delete",
            "application",
            f"{instance_name}.kubernetes.default.svc",
            "-n",
            ARGOCD_NAMESPACE,
        ],
        capture_output=True,
...
def delete_instance(instance_name: str, force: bool = False) -> None:
    ...
    if result.returncode != 0:
        ...
        instance_config = {"PHD_INSTANCE_NAME": instance_name}
