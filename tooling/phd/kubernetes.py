...
class KubernetesClient:
    ...
    def apply_manifest(
        ...
        for doc in filter(lambda x: x, documents):
            ...
            if api_version.startswith("argoproj.io/"):
                self._logger.debug("Using kubectl for Argo Workflows resource")
                self._apply_argo_workflows_resource(doc, namespace)
            else:
                self._logger.debug("Using utils.create_from_dict for standard resource")
                try:
                    ...
                    if not ("409" in str(e) or "AlreadyExists" in str(e)):
                        raise KubernetesError(f"Failed to apply manifest: {e}") from e
    ...
    def _apply_argo_workflows_resource(self, doc: dict, namespace: str) -> None:
        """
        Apply Argo Workflows resources using kubectl.
        ...
        try:
            ...
            if result.returncode != 0:
                if not ("already exists" in result.stderr or "409" in result.stderr):
                    raise KubernetesError(
                        f"Failed to apply Argo Workflows resource: {result.stderr}"
                    )
            ...
            raise KubernetesError(
                f"Failed to apply Argo Workflows resource: {e}"
            ) from e
        except Exception as e:
            raise KubernetesError(
                f"Unexpected error applying Argo Workflows resource: {e}"
            ) from e
