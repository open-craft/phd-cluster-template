...
def build_instance_config(  # pylint: disable=too-many-locals,too-many-positional-arguments
    ...
    if edx_platform_repository is not None:
        instance_config["PHD_EDX_PLATFORM_REPOSITORY"] = edx_platform_repository

    if edx_platform_version is not None:
        instance_config["PHD_EDX_PLATFORM_VERSION"] = edx_platform_version

    if tutor_version is not None:
        instance_config["PHD_TUTOR_VERSION"] = tutor_version
    ...
    instance_config.update(
        {
            "PHD_INSTANCE_MONGODB_DATABASE": config_data.get(
                "MONGODB_DATABASE", f"phd-{instance_name}-openedx"
            ),
            "PHD_INSTANCE_MONGODB_DATABASE_FORUM": config_data.get(
                "FORUM_MONGODB_DATABASE", f"phd-{instance_name}-forum"
            ),
    ...
    instance_config.update(
        {
            "PHD_INSTANCE_STORAGE_BUCKET_NAME": config_data.get(
                "STORAGE_BUCKET_NAME", ""
            ),
...
def load_instance_config(instance_name: str, logger: logging.Logger) -> dict:
    ...
    if not config_file.exists():
        ...
        return {"PHD_INSTANCE_NAME": instance_name}
